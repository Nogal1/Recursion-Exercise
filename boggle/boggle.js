...
function find(board, word) {
  ...
  function dfs(x, y, word) { // dfs = depth-first search
    if (word.length === 0) return true;
    if (x < 0 || x >= board.length || y < 0 || y >= board[0].length || board[x][y] !== word[0]) {
      return false;
    }
    ...
    // Move in all 4 directions: North, East, South, West
    const found = (
      dfs(x + 1, y, word.slice(1)) ||
      dfs(x - 1, y, word.slice(1)) ||
      dfs(x, y + 1, word.slice(1)) ||
      dfs(x, y - 1, word.slice(1))
    );

    board[x][y] = temp; // Unmark visited
    return found;
  }
